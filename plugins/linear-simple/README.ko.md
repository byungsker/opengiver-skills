...
`--global`은 `~/.openclaw/skills/linear-simple`에 설치합니다. `openclaw skills install`이 없는 구버전은 OpenClaw를 먼저 업데이트하거나 활성 workspace의 `skills/linear-simple`에 해당 디렉터리를 배치하세요.
